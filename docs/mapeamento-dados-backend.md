...
- ENEM: `backend/scripts/importar_enem_zip.py` importou 2.757 questoes unicas do `enem-api-main.zip` para `questoes`/`alternativas`, com etiquetas de serie `3ª série EM`, nivel `Médio`, materias/areas ENEM e marcadores `enem:ano:questao:idioma` em `competencias`.
